  ...
  return (
    <PageShell>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={VIEW} style={{ maxWidth: '920px', margin: '0 auto 48px' }}>
        ...
        <WordReveal text="Built for the way law students work" as="h2" className="serif section-title" style={{ textAlign: 'center' }} />
        <motion.div className="cards-grid" variants={staggerContainer(0.1, 0.05)} initial="hidden" whileInView="visible" viewport={VIEW} style={{ marginTop: '32px' }}>
          {OFFER_CARDS.map((item, i) => (
            <motion.div key={item.title} variants={staggerItem}>
              <TiltCard className="glass-card" style={{ display: 'flex', flexDirection: 'column', gap: '10px', height: '100%' }}>
                <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '0.85rem', color: 'var(--grey-text)' }}>0{i + 1}</span>
      ...
      <motion.section initial="hidden" whileInView="visible" viewport={VIEW} style={{ maxWidth: '780px', margin: '0 auto 48px', textAlign: 'center' }}>
        <p className="eyebrow" style={{ textAlign: 'center' }}>— Legal Insights</p>
        <WordReveal text="Every major domain of practice" as="h2" className="serif section-title" style={{ textAlign: 'center' }} />
        <motion.div style={{ display: 'flex', flexWrap: 'wrap', gap: '9px', justifyContent: 'center', marginTop: '20px' }} variants={staggerContainer(0.04, 0.1)} initial="hidden" whileInView="visible" viewport={VIEW}>
          {LAW_AREAS.map(area => (
      ...
      <style>{`
        .eyebrow { font-size: 0.76rem; font-weight: 600; letter-spacing: 0.08em; text-transform: uppercase; color: var(--grey-text); margin-bottom: 14px; }
